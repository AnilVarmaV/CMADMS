...
public class EdelCreteDocumentComponent {
	...
	public int edelCreateDocument(VWAttachment attachment,String workFlowNumber){
		...
		try{
			...
			System.out.println("Entered into folder creation:::::::::::statusValue::");
			...
			while (iter.hasNext()) {
				...
				}else if("In Progress".equalsIgnoreCase(statusValue) && !ID.equalsIgnoreCase(docID.toString())){
				...
				else if(!ID.equalsIgnoreCase(docID.toString()) && doCheckin){



					System.out.println("After else if Condition");
							...
							else
							{
								...
								for (int j = 0; j < parameterNames.length; j++) {
									...
									if(parameters.equalsIgnoreCase(config.getProperty(ASPConstants.ATTACHMENT_NAME))){
										...
										System.out.println("Doc attachment symbalicname"+config.getProperty(ASPConstants.ATTACHMENT_NAME));

										stepElement.setParameterValue(config.getProperty(ASPConstants.ATTACHMENT_NAME), att, true);
									}
									System.out.println("Step element parameter names is::::::::::::::::::::"+parameters); 
								}
								stepElement.doSave(true);
								System.out.println("StepName after setting to attachment::::::::::"+stepName);
							}
						}
					}

					Document doc = Factory.Document.fetchInstance(objStore,docID, null);
					//Document doc1 = Factory.Document.getInstance(objStore, "DMS", docID);
					doc.delete();
			...
			System.out.println(" noOfUniqDocsInSytem ::"+noOfUniqDocsInSytem);
